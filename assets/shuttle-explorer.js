(function () {
  ...
  var ACTIONS_COLUMN_LABEL = "Actions";
  var COPY_TABLE_BUTTON_LABEL = "Copy table to clipboard";
  var COPY_TABLE_DISPLAY_LABEL = "Copy table";
  ...
  function buildAmeriFluxBulkScriptText(siteEntries, options) {
    ...
    return [
      ...
      "TRACKING_DIR=\"${TRACKING_DIR:-$(dirname \"$LOGFILE\")}\"",
      "PREFIX_FILENAMES=\"${PREFIX_FILENAMES:-}\"",
      "AMERIFLUX_SUCCESS_FILE=\"${AMERIFLUX_SUCCESS_FILE:-$TRACKING_DIR/successful_downloads_ameriflux.tsv}\"",
      "AMERIFLUX_FAILED_FILE=\"${AMERIFLUX_FAILED_FILE:-$TRACKING_DIR/failed_downloads_ameriflux.tsv}\"",
      ...
      "",
      "mkdir -p \"$OUTDIR\" \"$TRACKING_DIR\"",
      "cd \"$OUTDIR\"",
      ": > \"$LOGFILE\"",
      ": > \"$AMERIFLUX_SUCCESS_FILE\"",
      ": > \"$AMERIFLUX_FAILED_FILE\"",
      "",
      "decode_base64() {",
      "  if command -v base64 >/dev/null 2>&1; then",
      "    if printf '%s' 'WA==' | base64 --decode >/dev/null 2>&1; then",
      ...
      "}",
      "",
      "ameriflux_filename_from_url() {",
      "  local url=\"$1\"",
      "  local site_id=\"$2\"",
      "  local data_product=\"$3\"",
      "  local data_policy=\"$4\"",
      "  local clean=\"${url%%\\?*}\"",
      ...
      "    base=\"download.bin\"",
      "  fi",
      "  if [ -n \"$PREFIX_FILENAMES\" ]; then",
      "    prefix=\"$(safe_token \"$site_id\")_$(safe_token \"$data_product\")_$(safe_token \"$data_policy\")\"",
      "    printf '%s_%s\\n' \"$prefix\" \"$base\"",
      "    return 0",
      "  fi",
      "  printf '%s\\n' \"$base\"",
      ...
      "  fi",
      "",
      "  RESPONSE_FILE=\"$(mktemp \"$TRACKING_DIR/ameriflux_response.XXXXXX\" 2>/dev/null || mktemp \"${TMPDIR:-/tmp}/ameriflux_response.XXXXXX\" 2>/dev/null || true)\"",
      "  if [ -z \"$RESPONSE_FILE\" ]; then",
      "    echo \"Could not create a temporary response file for ${SITE_ID} (${DATA_PRODUCT}); skipping.\" | tee -a \"$LOGFILE\"",
      "    printf '%s\\t%s\\t%s\\t%s\\n' \"$SITE_ID\" \"$DATA_PRODUCT\" \"$DATA_POLICY\" \"tempfile\" >> \"$AMERIFLUX_FAILED_FILE\"",
      "    continue",
      "  fi",
      "",
      "  if ! HTTP_STATUS=$(curl -sS -w \"%{http_code}\" -o \"$RESPONSE_FILE\" -X POST \"$REQUEST_URL\" \\",
      ...
      "      printf '%s\\t%s\\t%s\\t%s\\n' \"$SITE_ID\" \"$DATA_PRODUCT\" \"$DATA_POLICY\" \"request\" >> \"$AMERIFLUX_FAILED_FILE\"",
      "      continue",
      "  fi",
      "  RESPONSE=\"$(cat \"$RESPONSE_FILE\")\"",
      "  rm -f \"$RESPONSE_FILE\"",
      ...
      "      printf '%s\\t%s\\t%s\\t%s\\n' \"$SITE_ID\" \"$DATA_PRODUCT\" \"$DATA_POLICY\" \"http_${HTTP_STATUS}\" >> \"$AMERIFLUX_FAILED_FILE\"",
      "      continue",
      "    else",
      "      diagnostic_status=\"$?\"",
      ...
      "  if [ -z \"$URLS\" ]; then",
      "    if print_download_request_error \"$SITE_ID\" \"$DATA_PRODUCT\" \"$HTTP_STATUS\" \"$RESPONSE\"; then",
      "      printf '%s\\t%s\\t%s\\t%s\\n' \"$SITE_ID\" \"$DATA_PRODUCT\" \"$DATA_POLICY\" \"no_urls\" >> \"$AMERIFLUX_FAILED_FILE\"",
      "      continue",
      "    else",
      "      diagnostic_status=\"$?\"",
      "      if [ \"$diagnostic_status\" = \"2\" ]; then",
      ...
      "    [ -n \"$url\" ] || continue",
      "    filename=\"$(ameriflux_filename_from_url \"$url\" \"$SITE_ID\" \"$DATA_PRODUCT\" \"$DATA_POLICY\")\"",
      "    log_ameriflux_download \"$filename\"",
      "    echo \"Downloading ${filename} (${SITE_ID}, ${DATA_PRODUCT})\" | tee -a \"$LOGFILE\"",
      "    if curl -L \"$url\" -o \"$filename\"; then",
      "      printf '%s\\t%s\\t%s\\t%s\\n' \"$SITE_ID\" \"$DATA_PRODUCT\" \"$DATA_POLICY\" \"$filename\" >> \"$AMERIFLUX_SUCCESS_FILE\"",
      "    else",
      "      echo \"Download failed for ${SITE_ID} (${DATA_PRODUCT}): $url\" | tee -a \"$LOGFILE\"",
      "      printf '%s\\t%s\\t%s\\t%s\\n' \"$SITE_ID\" \"$DATA_PRODUCT\" \"$DATA_POLICY\" \"$url\" >> \"$AMERIFLUX_FAILED_FILE\"",
      "    fi",
      "  done <<< \"$URLS\"",
      "done < \"$SITES_FILE\"",
      "",
  ...
  function buildDownloadAllSelectedScriptText(options) {
    ...
    var lines = [
      ...
      "DATA_DIR=\"$OUTPUT_DIR/siteData\"",
      "COMBINED_LOGFILE=\"$LOG_DIR/download.log\"",
      "DIRECT_LOGFILE=\"$LOG_DIR/direct_links.log\"",
      "DIRECT_SUCCESS_FILE=\"$LOG_DIR/successful_downloads_direct.tsv\"",
      "DIRECT_FAILED_FILE=\"$LOG_DIR/failed_downloads_direct.tsv\"",
      "DIRECT_QUEUE_DIR=\"$LOG_DIR/direct_queue\"",
      "AMERIFLUX_SITES_FILE=\"$LOG_DIR/ameriflux_selected_sites.txt\"",
      "AMERIFLUX_LOGFILE=\"$LOG_DIR/ameriflux_api.log\"",
      "AMERIFLUX_SUCCESS_FILE=\"$LOG_DIR/successful_downloads_ameriflux.tsv\"",
      "AMERIFLUX_FAILED_FILE=\"$LOG_DIR/failed_downloads_ameriflux.tsv\"",
      ...
      "}",
      "",
      "count_successful_sites() {",
      "  # Counts unique site IDs. One site can have multiple products, but the final user-facing",
      "  # success total is intentionally site-level.",
      "  {",
      "    print_file_if_exists \"$DIRECT_SUCCESS_FILE\"",
      "    print_file_if_exists \"$AMERIFLUX_SUCCESS_FILE\"",
      "  } | awk -F '\\t' 'NF && $1 != \"\" { seen[$1]=1 } END { for (site in seen) count++; print count + 0 }'",
      "}",
      "",
      "count_failed_downloads() {",
      "  # Counts unique failed download/product records so multiple product failures for one site",
      "  # remain visible instead of being collapsed into a site-level total.",
      "  {",
      "    print_file_if_exists \"$DIRECT_FAILED_FILE\"",
      "    print_file_if_exists \"$AMERIFLUX_FAILED_FILE\"",
      ...
      "  local missing=0",
      "  local cmd",
      "  for cmd in awk bash curl basename cat date dirname find mkdir rm tee tr wc; do",
      ...
      "    for cmd in cksum grep mktemp mv sed sort; do",
      "      if ! command -v \"$cmd\" >/dev/null 2>&1; then",
      ...
      "    return 0",
      "  fi",
      "  log_combined \"Starting direct-link downloads into $DATA_DIR\"",
      "  if TRACKING_DIR=\"$LOG_DIR\" SUCCESS_FILE=\"$DIRECT_SUCCESS_FILE\" FAILED_FILE=\"$DIRECT_FAILED_FILE\" QUEUE_DIR=\"$DIRECT_QUEUE_DIR\" PREFIX_FILENAMES=1 bash -s -- \"$DATA_DIR\" \"$DIRECT_LOGFILE\" <<'FLUXNET_DIRECT_LINK_SCRIPT'"
    ];
    ...
    lines = lines.concat([
      ...
      "  log_combined \"Starting AmeriFlux API downloads into $DATA_DIR\"",
      "  if TRACKING_DIR=\"$LOG_DIR\" AMERIFLUX_SUCCESS_FILE=\"$AMERIFLUX_SUCCESS_FILE\" AMERIFLUX_FAILED_FILE=\"$AMERIFLUX_FAILED_FILE\" PREFIX_FILENAMES=1 bash -s -- \"$DATA_DIR\" \"$AMERIFLUX_SITES_FILE\" \"$AMERIFLUX_LOGFILE\" <<'FLUXNET_AMERIFLUX_SCRIPT'"
    ]);
    ...
    lines = lines.concat([
      ...
      "  successful_sites=\"$(count_successful_sites)\"",
      "  failed_downloads=\"$(count_failed_downloads)\"",
      "  if [ \"$failed_downloads\" -eq 0 ] && [ -n \"$FAILED_SECTIONS\" ]; then",
      "    failed_downloads=1",
      "  fi",
      "  FINAL_FAILED_DOWNLOADS=\"$failed_downloads\"",
      "  echo \"\"",
      "  log_combined \"Bulk download complete.\"",
      "  log_combined \"Sites downloaded: $successful_sites\"",
      "  log_combined \"Failed downloads: $failed_downloads\"",
      "}",
      "",
      "main() {",
      ...
      "  download_ameriflux_api_products",
      "  print_final_summary",
      "  if [ \"$FINAL_FAILED_DOWNLOADS\" -gt 0 ] || [ -n \"$FAILED_SECTIONS\" ]; then",
      "    exit 1",
      "  fi",
      "}",
      "",
  ...
  function ensureStyles() {
    ...
    style.textContent = [
      ...
      ".shuttle-explorer__data-notes{margin-top:12px;}",
      ...
      ".shuttle-explorer__attribution-row{display:flex;justify-content:space-between;align-items:center;gap:10px;margin:8px 0 0;}",
	      ...
	      ".shuttle-explorer__map-legend-swatch{display:inline-block;width:10px;height:10px;border-radius:999px;flex:0 0 auto;}",
		      ...
		      "@media (max-width: 640px){.shuttle-explorer__table th.shuttle-explorer__actions-col,.shuttle-explorer__table td.shuttle-explorer__actions-cell{width:132px;min-width:132px;max-width:146px;}.shuttle-explorer__actions-cell .shuttle-explorer__btn{padding-right:6px;padding-left:6px;}}"
    ].join("");
  ...
  function createLayout(root) {
    ...
    root.innerHTML = [
      ...
      "  <p class=\"shuttle-explorer__tiny\">Data surfaced by the Explorer remain governed by the original data providers' terms, licenses, citation requirements, and data-use policies. Follow the applicable policy for each downloaded dataset, and cite or acknowledge the original data providers, networks, and product DOIs where applicable. Use the data policy tools above to generate the citation table and acknowledgements text required by the FLUXNET data use policy.</p>",
      "</aside>",
      ...
      "  <p class=\"shuttle-explorer__tiny\"><strong>Citation:</strong> Keenan TF, 2026. FLUXNET Data Explorer (v1.0.0). Zenodo. <a href=\"https://doi.org/10.5281/zenodo.20331228\" target=\"_blank\" rel=\"noopener noreferrer\">https://doi.org/10.5281/zenodo.20331228</a><br><strong>Version:</strong> v1.0.0. <strong>License:</strong> Apache-2.0. <strong>Source code:</strong> <a href=\"https://github.com/trevorkeenan/fluxnet-data-explorer\" target=\"_blank\" rel=\"noopener noreferrer\">github.com/trevorkeenan/fluxnet-data-explorer</a>.</p>",
  ...
  Explorer.prototype.resetTableCopyButtonLabel = function () {
    ...
    button.textContent = COPY_TABLE_DISPLAY_LABEL;
    button.setAttribute("aria-label", COPY_TABLE_BUTTON_LABEL);
    button.title = COPY_TABLE_BUTTON_LABEL;
  ...
  Explorer.prototype.buildCurlScript = function (rows) {
    ...
    return [
      ...
      "",
      "mkdir -p \"$OUTDIR\" \"$TRACKING_DIR\"",
      "SUCCESS_FILE=\"${SUCCESS_FILE:-$TRACKING_DIR/successful_downloads_direct.tsv}\"",
      "FAILED_FILE=\"${FAILED_FILE:-$TRACKING_DIR/failed_downloads_direct.tsv}\"",
      "QUEUE_DIR=\"${QUEUE_DIR:-$TRACKING_DIR/direct_queue}\"",
      "mkdir -p \"$QUEUE_DIR\"",
      ": > \"$SUCCESS_FILE\"",
      ": > \"$FAILED_FILE\"",
      ": > \"$LOGFILE\"",
      ...
      "",
      "  if [ -n \"$PREFIX_FILENAMES\" ]; then",
      "    sid=\"$(safe_token \"$site_id\")\"",
      "    hub=\"$(safe_token \"$data_hub\")\"",
      "    if [ -n \"$hub\" ]; then",
      "      prefix=\"$hub\"",
      "    fi",
      "    if [ -n \"$sid\" ]; then",
      "      prefix=\"${prefix:+${prefix}_}$sid\"",
      "    fi",
      "    if [ -n \"$prefix\" ]; then",
      "      printf '%s_%s\\n' \"$prefix\" \"$base\"",
      "      return 0",
      ...
      "  local part_path=\"$tracking_dir/$filename.part\"",
      "  local tmp_headers=\"$tracking_dir/.headers.$$.tmp\"",
      "  local cookie_jar=\"$tracking_dir/.cookies.$$.txt\"",
      "",
      "  local ts",
      ...
      "      {} \"$OUTDIR\" \"$LOGFILE\" \"$attempt\" \"$SUCCESS_FILE\" \"$failed_out\" \"$TRACKING_DIR\" &",
      "    worker_pid=$!",
      ...
      "      download_one \"$entry\" \"$OUTDIR\" \"$LOGFILE\" \"$attempt\" \"$SUCCESS_FILE\" \"$failed_out\" \"$TRACKING_DIR\" || true",
      "    done < \"$in_file\"",
      "  fi",
      "",
